<div align="center">

# 👋 Hi, I'm 郑恩赐 (Zheng Enci)

**AI全栈应用开发 | 软件工程本科生 | 寻找实习/工作机会**

[![GitHub followers](https://img.shields.io/github/followers/Zheng-Enci?style=social)](https://github.com/Zheng-Enci)
[![GitHub stars](https://img.shields.io/github/stars/Zheng-Enci?style=social)](https://github.com/Zheng-Enci)

📌 **多平台同步**: [GitHub](https://github.com/Zheng-Enci) | [Gitee](https://gitee.com/zheng-enci050704) | [GitCode](https://gitcode.com/ZhengEnCi)

</div>

<table>
  <tr>
    <td width="60%">
      <picture>
        <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/Zheng-Enci/Zheng-Enci/master/dist/github-contribution-grid-snake-dark.svg">
        <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/Zheng-Enci/Zheng-Enci/master/dist/github-contribution-grid-snake.svg">
        <img alt="github contribution grid snake animation" src="https://raw.githubusercontent.com/Zheng-Enci/Zheng-Enci/master/dist/github-contribution-grid-snake.svg" width="100%">
      </picture>
    </td>
    <td width="40%" align="center">
      <img src="./assets/profile.gif" alt="Profile" width="160px">
    </td>
  </tr>
</table>

---

## 🚀 关于我

- 🎓 **厦门工学院** 人工智能学院 | 软件工程（AI方向）| 2023-2027
- 💻 专注于 **AI全栈应用开发**，热爱将AI技术落地到实际项目中
- 🌱 正在探索 **多模态大模型** 的应用与集成
- 🔭 目前正在寻找合适的实习/工作机会
- 🏆 **国家级大创立项** | **2项发明专利** | **掘金优质作者榜第2名**

---

## 🛠️ 技术栈

### 编程语言
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![Java](https://img.shields.io/badge/Java-ED8B00?style=flat&logo=java&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)
![TypeScript](https://img.shields.io/badge/TypeScript-007ACC?style=flat&logo=typescript&logoColor=white)

### 前端技术
![Vue.js](https://img.shields.io/badge/Vue.js-35495E?style=flat&logo=vue.js&logoColor=4FC08D)
![Element Plus](https://img.shields.io/badge/Element_Plus-409EFF?style=flat&logo=element&logoColor=white)
![ECharts](https://img.shields.io/badge/ECharts-AA344D?style=flat&logo=apache-echarts&logoColor=white)

### 后端技术
![Spring Boot](https://img.shields.io/badge/Spring_Boot-6DB33F?style=flat&logo=spring-boot&logoColor=white)
![Flask](https://img.shields.io/badge/Flask-000000?style=flat&logo=flask&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=flat&logo=mysql&logoColor=white)
![Redis](https://img.shields.io/badge/Redis-DC382D?style=flat&logo=redis&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-316192?style=flat&logo=postgresql&logoColor=white)

### AI/机器学习
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat&logo=pytorch&logoColor=white)
![YOLO](https://img.shields.io/badge/YOLO-00FFFF?style=flat&logo=yolo&logoColor=black)
![OpenCV](https://img.shields.io/badge/OpenCV-5C3EE8?style=flat&logo=opencv&logoColor=white)

### 部署运维
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white)
![Nginx](https://img.shields.io/badge/Nginx-009639?style=flat&logo=nginx&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat&logo=linux&logoColor=black)

---

## 📊 GitHub 统计

<div align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=Zheng-Enci&show_icons=true&theme=radical" alt="GitHub Stats" height="165">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Zheng-Enci&layout=compact&theme=radical" alt="Top Languages" height="165">
</div>

<div align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=Zheng-Enci&theme=radical" alt="GitHub Streak">
</div>

---

## 💼 实习经历

### 厦门弘尘网络科技有限公司 | 前后端开发工程师 | 2024.08

- 快速搭建人力资源管理信息系统基础框架，涵盖员工管理、考勤打卡、部门管理等核心模块
- 设计并实现JWT Token认证机制，采用Vue3 + Spring Boot + MyBatis + MySQL技术栈
- 深入理解前后端分离架构与RESTful API设计规范

---

## 🏆 项目经验

### [学生管理系统](https://10257qh6684dd.vicp.fun/) | 2025.09 - 2026.04
**技术栈**：Vue3 + Spring Boot 3.x + MySQL + Redis + Nginx

- 为厦门工学院AI创作坊开发的一体化学生管理系统
- **独立完成全栈开发**，涵盖7个核心模块
- 实现JWT认证 + IP白名单 + 验证码校验**三重安全机制**
- Redis缓存优化，**响应时间<100ms**
- [开源代码](https://github.com/Zheng-Enci)（前端开源）

---

### 课堂异常行为检测系统（EduBot）| 2025.03 - 2025.04
**技术栈**：PyTorch + YOLOv8n + Flask + MySQL + 通义千问/紫东太初

- **国家级大创立项**（项目编号：202513115006）
- **发明专利**（第三发明人）：CN202510645941.9
- 使用YOLOv8n + PyTorch实现4类行为检测
- 接入大语言模型实现PPTX转思维导图、教案生成
- [开源代码](https://github.com/Zheng-Enci)（前后端开源）

---

### 红星农场玉米出芽监测系统 | 2025.10 - 2025.11
**技术栈**：HTML5/CSS3 + Fabric.js + JavaScript + YOLO数据解析

- 承接**农科院委托项目**，服务北大荒集团
- 独立完成Canvas渲染、YOLO数据解析、热力图生成
- 行距误差控制在**5像素内**
- 巡田效率提升，全流程压缩至**10分钟内**

---

### 中科院城市环境高空摄像头多路图像拼接系统 | 2025.06 - 2025.07
**技术栈**：PyTorch + ResNet50 + OpenCV + CUDA GPU加速

- 承接**中科院城市环境研究所**委托项目
- 设计多图像拼接算法，支持4-5张图片拼接
- 优化双分支回归网络，拼接精度显著提升

---

### 其他项目

- **三维重建全流程文件智慧管理系统**（2025.09-2025.11）：Vue3 + TypeScript + Spring Boot + PostgreSQL，支持大文件上传（2GB）
- **ROS全栈自主控制小车**（2025.04）：Flask + ROS + OpenCV + RDKx5开发板
- **智慧监控系统**（2025.09-2026.02）：Python + OpenCV，多线程监控3台摄像机
- **IP监控系统**（2025.12）：Flask + Python threading，热力图可视化IP使用频率
- **mdconvert Python库**（2026.03）：Markdown转HTML/PDF/DOCX，已发布PyPI
- **鼠标和键盘模拟项目**（2026.03）：USB Gadget HID协议，FastAPI控制

---

## 🎖️ 荣誉与成就

- 🏆 **国家级大学生创新创业训练计划项目**立项（2025.09）
- 📜 **发明专利**（第三发明人）：基于多模态大模型的课堂行为检测方法（CN202510645941.9）
- 📜 **发明专利**（第二发明人）：一种基于课堂监控的学生行为检测系统及方法（CN202511244232.6）
- 🥈 **掘金优质作者榜第2名**（2025年第11期）
- 📋 **工业和信息化人才岗位能力认证证书**（2024.05）

---

## 📫 联系我

- 📧 **Email**: [zheng_enci@qq.com](mailto:zheng_enci@qq.com)
- 💼 **GitHub**: [@Zheng-Enci](https://github.com/Zheng-Enci)
- 📝 **掘金**: [郑恩赐](https://juejin.cn/user/2883382090934252)
- 📚 **CSDN**: [郑恩赐的博客](https://blog.csdn.net/2301_79239314)
- 🔗 **Gitee**: [zheng-enci050704](https://gitee.com/zheng-enci050704)

---

<div align="center">

**💡 正在探索多模态大模型的应用，欢迎交流！**

</div>
