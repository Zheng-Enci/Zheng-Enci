...
## 2025-10-28 ~ 2025-11-20 （本科三年级）| 项目：红星农场玉米出芽监测系统

### 项目背景
承接农科院委托，针对红星农场（隶属北大荒集团北安分公司，耕地面积超41万亩）玉米出芽监测需求，解决玉米出芽窗口短且人工巡田难以及时发现"缺苗带"的问题；需求是利用无人机航拍+YOLO检测，通过可旋转/可平移网格判定每格是否含苗，未变色区域即为补苗优先区。

### 项目目标
①支持上传航拍大图与YOLO标签，实时生成网格热力视图；②可调节网格尺度、旋转角与平移偏移，适配不规则田垄；③提供出芽率计算与人工校正能力，保障农场主在黄金补苗期内精准决策。

### 我的角色
全栈可视化开发（独立完成Canvas渲染、检测数据解析、交互逻辑与UX优化）

### 核心职责
- **YOLO数据解析与网格聚合**：解析YOLO txt，按网格聚合苗点并使用Fabric.js高亮含苗格，空白格即标记缺苗风险
- **自适应网格系统**：构建"网格大小/旋转/平移"多滑块，与画布缩放平移同步刷新，实现多角度对齐与临界行校准
- **人工复核机制**：通过`manualCellStates`与画布点击事件，支持人工标红或调整计数，形成"算法检测+人工复核"闭环，避免光照/杂草干扰
- **出芽率计算面板**：集成出芽率面板，自动统计检测苗数，输入播种总量即可秒级计算出芽率，辅助补苗优先级排序

### 技术栈/工具
- **前端技术**：HTML5/CSS3暗色面板、Fabric.js 5.3.0、原生JavaScript
- **数据处理**：YOLOv5/YOLOv8标注txt解析
- **样本数据**：`FTA03328.JPG`与`FTA03328_detections.txt`

### 项目成果
1. **效率提升**：将"无人机检测→网格热力→人工复核"链路压缩到10分钟内，值守人员可快速定位未变色格并派人补苗
2. **精度优化**：自适应网格旋转/平移+手工标注使行距误差控制在5像素内，解决长条田垄倾斜导致的定位偏差
3. **决策支持**：出芽率看板实时刷新，为农场主提供"缺苗热力 + 出芽率曲线"双重依据，显著降低人工巡田成本
...
## 掘金技术博客数据更新 | 2025-12-4

**最新数据统计：**
- **掘力值：** 1351分（较之前21分大幅提升）
- **文章曝光：** 157,134次
- **文章阅读：** 10,516次
- **文章点赞：** 51次
- **文章评论：** 5次
...
**数据增长亮点：**
- 掘力值从21分增长至1351分，增长64倍，体现了内容质量和社区活跃度的显著提升
- 文章曝光量突破15万次，内容传播效果显著
- 文章阅读量突破1万次，读者关注度持续提升
- 文章收藏量达到85次，用户认可度不断提升

**技术影响力提升：**
- 通过持续的技术内容输出，在掘金平台建立了良好的技术影响力
- 文章质量和实用性获得社区认可，数据指标持续向好
- 在Java、Python等技术领域的内容分享获得广泛关注
